...
/**
 * Expression for IS NOT TYPED predicate.
 * Checks if the value of an expression does NOT match the target type.
 * Returns true if the value's runtime type does NOT match the target type.
 * NULL values return true (NULL is not typed as anything specific).
 */
public class IsNotTypedExpression extends AbstractNonLeafExpression {

    private final IType<?> targetType;

    public IsNotTypedExpression(Expression input, IType<?> targetType) {
    ...
    public Object evaluate(Row row) {
        ...
        // NULL values are considered NOT typed
        if (value == null) {
            return true;
        }
        
        // Check if the value's type does NOT match the target type
    ...
    public String showExpression() {
        return inputs.get(0).showExpression() + " IS NOT TYPED " + targetType.getName();
    }

    @Override
    public Expression copy(List<Expression> inputs) {
        assert inputs.size() == 1;
        return new IsNotTypedExpression(inputs.get(0), targetType);
    }
